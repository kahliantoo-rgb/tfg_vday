...
flutter build apk --release --flavor production --build-name=1.0.3 --build-number=4
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$src = Join-Path $repoRoot "build\app\outputs\flutter-apk\app-production-release.apk"
$dest = Join-Path $repoRoot "build\app\outputs\flutter-apk\TFG-VDAY-v1.0.3-production.apk"
...
Write-Host "  $dest"
Write-Host ""
Write-Host "Install on phone, then confirm Dashboard title shows v1.0.3 (4) and bell icon in app bar."
